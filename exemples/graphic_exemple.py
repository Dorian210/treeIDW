...
# shape (N_boundary, 2) 2 for the 2D space
boundary_nodes = np.stack((x, y)).T
# shape (N_boundary, 2) 2 beacause the field is a vector field
boundary_field = np.stack((u, v)).T
# shape (N_internal, 2) 2 for the 2D space
internal_nodes = np.stack((xi, yi)).T

# shape (N_internal, 2) 2 beacause the field is a vector field
uivi = treeIDW(boundary_nodes, boundary_field, internal_nodes, parallel=True)
ui, vi = uivi.T
