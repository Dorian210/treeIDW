...
from treeIDW.helper_functions import compute_weight_vectorized, bisect_weight, inv_dist_weight, compute_weight_vectorized_parallel, bisect_weight_parallel, inv_dist_weight_parallel

def treeIDW(boundary_nodes: npt.NDArray[np.float64], 
            ...
            parallel: bool=False) -> npt.NDArray[np.float64]:
    """
    ...
    d_min_vals, closest_nodes_inds = tree.query(internal_nodes, workers=(-1 if parallel else 1))
    d_max = np.linalg.norm( np.minimum(boundary_nodes.min(axis=0), internal_nodes.min(axis=0)) 
    ...
    search_radii_squared = bisect_weight_(d_min_vals*d_min_vals, d_max*d_max, lowest_relevant_weights, bisect_rtol)
    del d_min_vals, d_max, lowest_relevant_weights
    
    search_radii = np.sqrt(search_radii_squared)
    del search_radii_squared
    
    relevant_nodes_inds = tree.query_ball_point(internal_nodes, search_radii, workers=tree_workers)
    ...
    internal_field = inv_dist_weight_(boundary_nodes, boundary_field, internal_nodes, 
                                      relevant_nodes_inds_flat, relevant_nodes_inds_sizes)
